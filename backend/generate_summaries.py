...
# 3. Main Script
# ─────────────────────────────────────────────────────────────────────────────
def generate_summary_for_file(doc_path: str, conn) -> str:
    """
    Generates a structured summary for a single document path (e.g. 'documents/filename.pdf')
    and saves it to the database using the provided psycopg2 connection.
    """
    ...
    if not chunks:
        raise ValueError(f"No chunks created for {doc_path}")
    ...
    if not chunk_summaries:
        raise ValueError(f"All chunk summaries failed for {doc_path}")
...
def main():
    ...
    for idx, doc_path in enumerate(to_process, 1):
        ...
        try:
            generate_summary_for_file(doc_path, conn)
